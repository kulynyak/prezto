...
# time agnostic
change_theme() {
  # default_theme=${1:-onehalf-light}.yaml
  default_theme=${1:-kitty-dark}.yaml
  is_night && default_theme=${1:-pencil-dark}.yaml
  gsed -e "/#+begin_theme/,/#+end_theme/c\#+begin_theme\n $(gsed -e 's/$/ \\/' ~/.config/alacritty/themes/$default_theme) \n#+end_theme" $alacritty_conf_dir/alacritty.yml -i
}
...
list_themes() {
  if check_if_themes_exists; then
    ...
    echo "-----------------------------"
    ls -1 $alacritty_themes_dir/*.yaml | gsed -r 's/(.*)\/(.*).yaml/    \2/g'
  else
